...
app.post('/api/students/bulk', authMiddleware, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    for (const s of students) {
      ...
      await client.query(
        `INSERT INTO students (id, name, nis, class_id, user_id) VALUES ($1, $2, $3, $4, $5)
                 ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, nis = EXCLUDED.nis, class_id = EXCLUDED.class_id`,
...
// ========== SPA FALLBACK ==========
// Note: Express 5 requires named parameters, so we use catch-all middleware
app.use((req, res, next) => {
